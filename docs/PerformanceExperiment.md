...
This document details an experiment to measure runtime performance of the endpoint `POST /api/scores` directly after [commit 9b7453c](https://github.com/valarao/wordle-solver/tree/9b7453cbf10e98cba9e457d6dff2a7c7c1050b3a).  

## Motivation

[Commit 9b7453c](https://github.com/valarao/wordle-solver/tree/9b7453cbf10e98cba9e457d6dff2a7c7c1050b3a) marks the completion of the core business logic of the `POST /api/scores` endpoint. 

Currently, a main requirement of the endpoint would be to serve the planned React site. The endpoint must be time-performant enough to serve common requests within 5.0 seconds. The results of this experiment aim to inform next steps. While a sample size of 10 games per strategy is small, this experiment can help validate the idea that better entropy scores leads to better guesses on average. 

Based on the experiment, one of two strategies will be followed:
...
A few major determinants that influence execution time:
- **Guess quality**: The guess quality (i.e. the bit score) reduces the space of possible answers.
- **Number of words considered**: Size of the words dataset mainly influences the performance of the first guess. This is because there is no prior information to pre-filter. This is also a major determinant when the guess quality is low.

## Methodology

The plan is to simulate 10 games of Wordle using 6 different strategies and record the execution time:

1. **Reduced Dataset, 99th Pick**
...
The randomly chosen words for each game are:
...
- Line Graph: Guess # vs. Possibilities (each Scenario)
- Runtime: Correlation between execution time and possible answers 

Scenario 6 used the full dataset of 10,657 words (including non-answers). This scenario run timed out after attempting to get the initial entropy values for 300 seconds.
...
### Scenario 6: Full Dataset

**Target: CIRCA**

| Guess #   | Word Chosen | Entropy Value | Bit Value | Execution Time   | Possible Answers |
| --------- | ----------- | ------------- | --------- | ---------------- | ---------------- |
| 0         | -           | -             | -         | -                | -             |
